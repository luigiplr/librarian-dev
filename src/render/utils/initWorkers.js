/*
 * Summary:     Initialize Node workers & spawn socketIO server on a available port
 * Parameters:  None
 * Return:      EventEmitter: 'initiated' Event
 * Author:      Luigi Poole
 */
class InitWorkers extends EventEmitter {
  ...
  initSocketEvents() {
    ...
    this.socket.on('connection', socket => {
      ...
      socket.on('daemon', ({ id, data }) => this.emit(id, data))
      socket.on('daemon:error', ({ id, error }) => this.emit(`${id}:error`, error))
